	...
	$.wysiwyg.controls.image = {
		init: function (Wysiwyg) {
			...
			formImageHtml = '<form class="wysiwyg"><fieldset><legend>{legend}</legend>' +
				'<label>{preview}: <images src="" alt="{preview}" style="float: left; margin: 5px; width: 80px; height: 60px; border: 1px solid rgb(192, 192, 192);"/></label>' +
		...
		processInsert: function (context, Wysiwyg, img) {
			...
			if (img.self) {
				// to preserve all images attributes
				...
				image = "<images src='" + url + "' title='" + title + "' alt='" + description + "'" + style + "/>";
		...
		makeForm: function (form, img) {
			...
			form.find('images').attr("src", img.src);

			form.find('images').bind("load", function () {
				if (form.find('images').attr("naturalWidth")) {
					form.find('input[name="naturalWidth"]').val(form.find('images').attr("naturalWidth"));
					form.find('input[name="naturalHeight"]').val(form.find('images').attr("naturalHeight"));
				}
			});

			form.find("input[name=src]").bind("change", function () {
				form.find('images').attr("src", this.value);
			});
	...
	$.wysiwyg.insertImage = function (object, url, attributes) {
		return object.each(function () {
			...
			if (attributes) {
				...
				image = Wysiwyg.getElementByAttributeValue("images", "src", "#jwysiwyg#");
