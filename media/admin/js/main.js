...
jQuery(function($){
	...
	return false;
      ...
      $('.save_razdel_input').on('keyup', function(){
      ...
      $('#save_and_edit').on('click', function(){
      
          $('#go_edit').val('1');
          
          $('#form_save').attr('target','_blank');
    ...
    if ($('#doctors_edit').length){
        $('.doc_choose').bind('change', function(){
            if ($(this).attr('checked') != undefined && $(this).attr('checked').length)
                checked = 1;
            else
                checked = 0;

            $.post('/ajax/doctors_page_save/', {page_id: $(this).data('page_id'), doctor_id:$(this).data('doctor_id'), checked:checked}, function(){
